...
もうひとつの売りが、無料枠の集約と、送るトークン量の削減だ。README には各社の無料枠を一覧にした画面があり、月あたりどれだけ使えるかを集計して表示すると書かれている。削減のほうは自前の仕組みで、README は RTK や Caveman といった先行プロジェクトの考え方を取り入れたものだと書いている。複数の圧縮を重ねて掛ける形で、看板に出ている数字は 15〜95%。ただし内訳を見ると幅がある。既定の組み合わせ（RTK → Caveman）を重ねたときは 78〜95%、平均 89% という計算が示されていて、**看板の平均89%はこの組み合わせの数字**だ。下限の15%は、いちばん軽い設定のときの値になる。鍵は手元に暗号化して置き、使用量や費用は自前の画面で見る、という作りになっている。


## 図

<svg viewBox="0 0 800 450" role="img" aria-label="見出しに「窓口はひとつ。止まったら次へ」とある図。左に「手元のツール エディタ・CLI（接続先は1つだけ）」があり、中央の「OmniRoute 自分のPCで動く窓口 19通りの振り分け方（送る量を削ってから流す）」を通る。右は「上から順に試す」で、1. 契約しているもの、2. 自分のAPIキー、3. 安いところ、4. 無料枠 が並ぶ。下に「上限・障害・費用のいずれで止まっても、ツール側の設定は変えないまま次へ移る」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
