...
判定のために全文を解析しない、というのがこのライブラリの設計です。読むのは相互参照テーブルとページツリーだけ。あとはコンテンツストリームを拾い読みして、**文字を置く命令（`Tj` / `TJ`）が出てくるか、画像を貼る命令（`Do`）が出てくるか**を見ます。結果は「テキスト」「スキャン」「画像」「混在」の4つに分けられ、確信度の数値が付きます。ここまでが、おおよそ 10〜50ミリ秒です。
...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「PDFを開き切る前に仕分けて、重い処理を必要な枚数に」とある図。左に「PDFの束（中身はまちまち）」があり、矢印で「pdf-inspector（構造だけ見る・10〜50ms）」に進む。そこから道が2つに分かれ、上は「そのまま Markdown へ（手元で完結する・追加の費用なし）」、下は「OCR に回す（重い・ページ単位で・手元でも走る）」につながる。下に「〜 全部を重い処理に流していたのを、必要なぶんだけに絞る 〜」とある。">
    ...
    <text x="607" y="330" text-anchor="middle" style="font-family: var(--jp); font-size: 15px; fill: #6E6A5F;">（重い・ページ単位で・手元でも走る）</text>
...
分かれ道が前に来ているのが要点です。上の道に乗ったぶんは重い処理を呼ばずに済むので、費用も時間も、下に流れた枚数ぶんだけで済みます。
...
**PDF ライブラリの置き換えではありません。** 狙いは仕分けと、素直な文書の Markdown 化までです。README が向いている相手として挙げているのは、文字データを持つ PDF のうち、速度・読み順・表の構造が要るもの。汎用の PDF 操作ライブラリの代わりになるものとしては書かれていません。
...
**OCR は使えますが、経路ごとに条件が違います。** Python と Node のパッケージには OCR の連携が入っていて、PP-OCRv6 Small を**手元で**走らせられます。Rust とコマンドラインから使う場合は、ビルドのときに OCR を有効にする必要があります。Rust の既定のビルドは OCR なしの抽出だけで、ブラウザ版にはそもそも OCR の経路がありません。

自分で用意するのは PDFium と ONNX Runtime の2つで、置き場所を環境変数で教えます。モデルのほうは用意する必要がなく、最初に OCR へ回ったページで自動的に取得されます（3点で約31MB、中身を検証したうえで手元に残ります）。どちらも、OCR に回るページが出なければ触られません。なお説明書きは、通しで動作を確認しているのは Linux の x64 で、macOS と Windows は preview 扱いだと断っています。
