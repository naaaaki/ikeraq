...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「テーマ1つを、工程を全部通して動画にする」とある図。「テーマ（一言）」から、「台本」「素材」「読み上げ・字幕」「音楽・書き出し」の4つの箱を順に通り、「3つの比率（縦・横・正方形）」の動画になる。前の3つの箱から点線が伸び、「この3つは担当を差し替えられる（無料のものから有料のものまで）」と添えてある。下に「〜 手が止まるのは編集ではなく、工程の間の受け渡しだった 〜」とある。">
