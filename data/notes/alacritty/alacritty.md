...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「持たない機能は、外に分ける」とある図。中央に「Alacritty 文字を描く（GPUで描画する）」があり、外に「tmux（画面の分割・セッション）」と「ウィンドウマネージャ（ウィンドウの配置）」が置かれている。下に「〜 作らない機能を決めることが、速さの取り方になっている 〜」とある。">
