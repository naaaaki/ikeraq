...
題材を一言書くか、手元の資料を投げ込むと、**授業まるごとを組み立てて再生する**ソフトです。清華大学の研究グループが公開しています。受け取れるのは PDF、Word、PowerPoint、表計算のほか、音声・動画・画像・テキストまで（音声と動画は、ffmpeg か対応サービスを別に用意しないと取り込みに失敗します）。中身を読んで目次を立て、その一項目ずつを「場面」に変えていきます。
...
出てきたものは持ち出せます。編集できる PowerPoint、単体で開ける HTML、授業まるごとの書き出しに対応しています。もう一方のモードである Pro workbench では、生成の途中で AI と会話しながら「このページを直して」と指示を出せます（**既定では無効**で、設定で有効にする必要があります）。

## 図

<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「資料1つが、場面の並びに変わって再生される」とある図。題材・資料（PDF・動画も。動画は別途用意が要る）が、目次（読んで組み立てる）を経て、4種類の場面（4つの形に分かれる）になり、教室として再生（複数のAIが役割を分ける）に至る。下に「〜 出力は資料ではなく、時間の流れを持った授業 〜」とある。">
    ...
    <text x="97" y="310" text-anchor="middle" style="font-family: var(--jp); font-size: 14px; fill: #6E6A5F;">（PDF・動画も。動画は別途用意が要る）</text>
...
**出来のばらつきはモデル側に出ます（記事の見立て）。** README がこの点について書いているのは推奨モデルまでで、「品質と速度の釣り合いがよい」ものと、「いちばん品質は高いが遅い」ものを挙げています。深い対話モードは 3D やシミュレーション、ゲーム、プログラミングまで生成する仕組みなので、使うモデルで仕上がりが変わることは見込んでおいたほうがよさそうです。
