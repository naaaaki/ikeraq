...
複数のコーディングエージェントをまとめて動かすためのデスクトップアプリです。README は「ターミナルで動くものなら何でも」と掲げていて、名前が挙がっているのは Claude Code、Codex、Copilot など29種類。どれか一つに寄せるのではなく、**同じ課題を何本か並べて走らせることを前提に作られています。**
...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「ひとつの課題を並べて走らせ、勝った一つを取る」とある図。左に「課題（1つ）」があり、「エージェント3体（作業ツリーが別なので踏み合わない）」に分かれて worktree A、worktree B、worktree C で動く。右で「差分を見比べて 1つをマージ（残りは捨てる）」に至る。下に「〜 どれが当たるか分からないなら、当ててから選ぶ 〜」とある。">
...
**まだ新しい道具です。** 名前が挙がっているだけで29種類のエージェント、内蔵ブラウザ、スマホアプリ、と守備範囲が広く、そのぶん個々の作り込みの深さは実際に触って確かめる必要があります。ライセンスは MIT です。
