...
<svg viewBox="0 0 800 430" role="img" aria-label="見出しに「書く前に手順を通す。AIの判断では飛ばさせない」とある図。左に「会話の開始（決まりを差し込む）」があり、使えそうなスキルは必ず使う と書かれている。中央の大きな箱は「決まった順に進む（AIの判断では飛ばせない）」で、詰める（質問して設計を固める）、計画（2〜5分の作業に割る）、実装（作業ごとに新しい担当）、確認（計画どおりか → 健全か）が並び、確認から実装へ戻る矢印に「落ちたら直して再確認」とある。右は「仕上がり（確かめ済み）」で、テストが通った作業用の枝について合流・PR・保持から選ぶ、と書かれている。下に「手順はセッション開始時に差し込まれ、使うかどうかをエージェントに選ばせない」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
  ...
  <text x="718" y="282" text-anchor="middle" font-size="12.5" fill="#1E5A48" font-weight="700">合流・PR・保持から選ぶ</text>
