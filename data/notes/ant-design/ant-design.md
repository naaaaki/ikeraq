...
**いまの本流は v6 です。** React は 18 以上が必要になりました。性能をさらに詰めたいとき向けに、実行時の組み立てをやめる `zeroRuntime` という指定も `ConfigProvider` のテーマに入っています。

## 図

<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「部品の下に、決めごとが敷いてある」とある図。上に「あなたの画面（作るもの）」があり、その下に「antd（テーブル・フォーム・日付入力などの部品）」、いちばん下に「Ant Design（余白・色・文言の決めごと）」が積まれている。右の「Design Token（色・角丸・余白）」から層に向かって矢印が伸びている。下に「〜 部品を借りると、判断の基準も一緒についてくる 〜」とある。">
...
**規模が大きい分、乗り換えのコストも大きくなります。** 部品の作り込みが深いので、一度これで作った画面を別のライブラリに移すのは簡単ではありません。v5 から v6 への移行のように、大きなバージョン更新のときにも作業が発生します（v6 では React 18 以上が要るようになりました）。採用は「長く付き合う」判断として決めたほうがいいです。

**議論は中国語が中心のようです。** Issue やディスカッションは中国語のやりとりが目立ち、込み入った不具合を追うときに言語の壁を感じることがあります。ドキュメント自体は英語版が整備されています。

**スタイルは既定で実行時に組み立てます。** CSS-in-JS はテーマの差し替えを軽くする代わりに、描画時の処理と、サーバーサイドレンダリング側の手当てが必要になります。表示速度に厳しい一般公開向けのページでは、そこを確かめてから決めたほうがいいです（v6 の `zeroRuntime` を使えば実行時の組み立てを避けられますが、そのぶん CSS を自分で読み込む必要があります）。
