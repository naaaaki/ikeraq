...
撮ったままの動画を入れたフォルダで、Claude Code のようなコーディングエージェントに「これを紹介動画にして」と話しかけると、完成した mp4 が返ってくる。言い淀み（「えーと」「あの」）や撮り直しのあいだの無音を落とす、区間ごとに色を整える、切れ目に30ミリ秒の音声フェードを入れる、字幕を焼き込む——といった作業が、プリセットやメニューを触らずに進む。特定のジャンル向けの決め打ちは持っておらず、対談でも旅行でもチュートリアルでも同じ道具で扱う設計になっている。
...
<svg viewBox="0 0 800 450" role="img" aria-label="見出しに「映像ではなく文字起こしの上で、編集を決める」とある図。左に「素材動画（撮ったまま）」があり、「文字起こし（約12KBのテキスト）」、「LLM が判断（どこを切るか）」、「完成 MP4（ffmpeg が書き出す）」と順につながる。途中に「自分で採点して、直しが要れば戻る（最大3回）」と添えてある。下に「迷った箇所だけ、コマ送りと波形を1枚のPNGにして見せる。全フレームは渡さない」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
...
**まだ新しい。** 2026年4月に公開されたばかりで、リリースのタグはまだ切られていない。参加している人も8名ほどで、未解決のものが issue とプルリクエスト合わせて90件あまり残っている（その大半はプルリクエスト）。ライセンスは MIT なので使うぶんの制約は緩いが、**仕様が動く前提で触るものだと思っておくほうがよい。**
