...
DeepSeek が公開した、AIエージェントの土台（ハーネス）です。Claude Code や Cursor のような「AIにコードを書かせる道具」の、中身にあたる部分だと考えると近いです。8月13日の公開から短い期間で、多くのスターを集めました。
...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「中核がない。だからぜんぶ差し替えられる」とある図。左は「ふつうのエージェント」で、中核には触れない、拡張は周りに足すだけ と書かれている。右は「DeepSeek Harness」で、モデル、道具、記録、設定、画面、考える輪 が並び、すべて対等に並ぶ、考える輪も、ただの部品 と添えてある。下に「〜 触れない部分が、ひとつもない 〜」とある。">
...
**評価が固まるにはまだ早い時期です。** 短い期間でこれだけのスターが集まったのは、中身が検証された結果というより、DeepSeek という名前への注目が大きいと見るのが自然です。リリースは**すべて試験版の扱い**です。なお GitHub の Issue 機能はこのリポジトリでは無効にされていて、不具合や要望の受け口は Discussions のほうに置かれています。「未解決の Issue が0件」に見えるのは、そのためです。
