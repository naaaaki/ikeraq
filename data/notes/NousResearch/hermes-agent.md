...
込み入った作業を片づけたあと、その手順を**再利用できる「スキル」として自分で書き出します**。置き場所は、Linux と macOS なら `~/.hermes/skills/` で、形式は agentskills.io という共通の規格に合わせてあります。作りっぱなしにはならず、実際に使いながら中身が磨かれていく作りです。過去のやりとりのほうは、セッションの記録を全文検索し、要約して引き出します。同じ相手と何度もやりとりするうちに、その人に合わせた理解が積み上がっていく、という説明がされています。

動かす先も入口も固定されていません。モデルは Nous Portal、OpenRouter、OpenAI などから選べて、コマンド1つで切り替えられます。動かす先はローカル、Docker、SSH、Modal などから選べます。入口は CLI のほか、Telegram、Discord、Slack、WhatsApp、Signal、メールなどに対応していて、**同じ1体のエージェントに複数の入口から話しかけられます**。

## 図

<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「一周するたびに、手持ちが増える」とある図。頼む、こなす、手順として書き出す、次はそれを使う、の4つが時計回りに円を描いて並び、円の中心に ~/.hermes/skills/（貯まっていく場所）がある。下に「〜 貯まるのはモデルの賢さではなく、自分で読める手順のファイル 〜」とある。">
...
「頼む → こなす」で切れる輪と、下半分までつながっている輪の違いです。本家は、こうした学習のループを内蔵しているのは自分たちだけだと言っています。ほかのエージェントが実際にどうかまでは確かめていません。
