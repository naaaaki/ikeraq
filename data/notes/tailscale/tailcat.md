...
仕組みの中心は、Tailscale を**データプレーンとコントロールプレーンに分けて、後者を使っていない**ところにあります。ふだんの Tailscale では、「誰と誰がつながってよいか」「相手はどこにいるか」を管理サーバ（コントロールプレーン）が教えます。tailcat はその役目を、**管理の外での受け渡し**に置き換えました。サーバ側でコマンドを打つと短い文字列が1つ表示されるので、それをチャットなり口頭なりで相手に渡す。README はこれを **tailcat address** と呼んでいます。中には WireGuard の公開鍵と、それとは別の事前共有鍵が入っていて、どの DERP 中継にいるかも含まれます。**この文字列を知っていること自体が、つないでよい証明になります。** 渡し方は帯域外であれば何でもよく、チャットでも口頭でも構いません。
...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「相手の居場所は人が手渡す。あとは2台だけで通す」とある図。左から順に、tailcat address（人が手で渡す）、DERP（まずここで出会う）、NAT越え（穴をあける）、直接つながる（失敗してもDERPが残る）が並ぶ。下に「〜 中継は借りる。ただし「誰とつなぐか」を決める管理サーバは無い 〜」とある。">
    ...
    <!-- ラベル -->
    <text x="124" y="292" text-anchor="middle" style="font-family: var(--jp); font-size: 19px; font-weight: 700; fill: #17160F;">tailcat address</text>
...
社外の人や、自分の管理下にない環境が相手だと、「まず同じサービスに登録してもらう」が最初の壁になります。tailcat は渡すものが短い文字列1つなので、その壁を飛ばせます。
...
**渡し方が、そのまま安全性になります。** 管理サーバが「誰とつなぐか」を判断しない設計なので、この文字列が漏れた相手＝つながれる相手です。README も「これは本来は秘密で、知っていることが接続できる理由になる」と念を押しています。既定では毎回その場かぎりの鍵が作られ、プロセスが終われば二度と使えなくなりますが、鍵をファイルに保存すると同じ文字列が使い回されるので、過去に渡した相手が将来のサーバにもつなげる状態が残ります（`serve --allow` でつなげる相手を絞ることはできます）。
