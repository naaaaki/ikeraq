...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「作らせる前に、問い詰める工程を挟む」とある図。左から「ぼんやりした依頼（人の頭の中だけにある）」、「/grill-me（人に質問を返す）」、「/to-spec → /to-tickets（読み返せる形にする）」、「/implement（作る）」が緑の矢印でつながっている。「？」の印と「合意ができるまで」の添え書きがある。下に「〜 手戻りは、書いたコードの中ではなく、依頼の曖昧さから生まれる 〜」とある。">
