...
動かしたあとの面倒も引き受ける。モデルは要求が来たときに読み込まれ、使われていないときやメモリが埋まってきたときに降ろされる（"loaded on request, unloaded when idle or memory fills"）。速度に効く設定 — README が名前を挙げているのは投機的デコードで、あとは「ほか」とまとめられている — も、機械に合わせて設定済みの状態で渡される。ここは、自分で詰めようとすると時間を取られやすいところでもある。

つなぎ先は、いま使っているエージェントのまま。README が名前を挙げているのは Pi、OpenCode、Hermes、OpenClaw、Codex、Claude Code、Oh My Pi、Cline で、付属の実行環境を使うこともできる（これは README ではなく公式ドキュメントに書かれている）。導入は npm のコマンド1つと `magnitude setup` の2手。エージェント自身に手順を読ませて設定させるやり方も併記されている。対応するのは macOS と Linux、Windows は WSL 経由。


## 図

<svg viewBox="0 0 800 450" role="img" aria-label="見出しに「モデルを選ぶ前に、機械を測る」とある図。左に「このPC（チップ・メモリ）（帯域）」があり、矢印で「合うモデルを提示（推定の毎秒トークン数つきで）」に進み、次に「落として動かす（設定は調整済み。使わなければ降ろす）」、最後に「エージェント（いつものものでよい）」につながる。下に「選ぶ・落とす・速度を詰める、の3つが1本につながっている」「やりとりは外に出ないが、そのぶん速度は機械の性能がそのまま出る」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
...
**中で何を使って推論しているかは、README に書かれていない。** 投機的デコードという調整項目の名前は出てくるが、推論エンジンの名前は出てこない。**基盤を自分で確かめてから入れたい場合は、READMEの外まで読みに行くことになる**（公式ドキュメントには、推論部分を Rust と llama.cpp の上に書いたと記されている）。

**まだ新しい。** 公開は2026年6月。書き手は少人数だが、リリースは高い頻度で続いている。作りが変わる可能性を前提に、**使い方を固めすぎない**のが無難になる。
