...
中身は、`specify` というコマンドラインの道具と、エージェントに登録される一連のコマンドでできている。プロジェクトを初期化すると、Claude Code や GitHub Copilot など使っているエージェントに `/speckit.〜` という手順用のコマンドが入る。柱になる手順は、プロジェクト全体の原則を決める**憲章**、何を・なぜ作るかを書く**仕様**、技術の選び方と構成を決める**計画**、それを細かい作業に割る**タスク**、**実装**、そして出来たコードを仕様・計画・タスクと突き合わせて残りを洗い出す**収束**。README は、収束が「済んだ」と言うまで実装と収束を繰り返す進め方を示している。その間に、曖昧な点を先につぶす確認や、書類どうしの食い違いを調べる分析などを、必要に応じて挟める。一段ごとに Markdown の書類が残り、次の段はその書類を読んで進む。仕様を書き始めると機能ごとに番号付きのフォルダができ（Git のブランチを切るかどうかは任意で、拡張を入れたときだけ自動で切られる。フォルダ名とブランチ名も結び付いていない）、計画の段では調査メモやデータの形、外に公開する口がある場合はその取り決めまで、別々の書類として出てくる。

仕様を作るコマンドの立て方は、推測をなくすことではなく、**推測したことを書き残させる**ほうにある。仕様を作るコマンドの定義は「文脈と業界の通例から、筋の通った推測をせよ」と指示したうえで、人に聞き返す「要確認」の印は**最大3件まで**と上限を切っている。4件以上になったら、影響の大きい順に3件だけ残して、あとは推測で埋めよ、という決め方だ。埋めたぶんは「前提」の節に書き出され、印が残っているうちは仕様が仕上がったことにならない。計画の段では、憲章に書いた原則（たとえば「最初は構成を増やしすぎない」「テストを先に書く」）に照らした関門があり、外れるなら理由を書かせる。対応しているエージェントは30種類以上。手順を足す拡張や、既存の手順を社内の書式に合わせて差し替えるプリセットの仕組みもある。


## 図

<svg viewBox="0 0 800 440" role="img" aria-label="見出しに「コードの前に、書類を3段通す」とある図。左に「作りたいもの」（ことばで書く）があり、緑の矢印で仕様（何を・なぜ）、計画（どう作るか）、タスク（細かい作業）の3つの書類へ順に進み、最後に実装（AIが書く）に着く。仕様の下には「聞き返すのは3件まで」「残りは推測して前提を書く」、計画の下には「原則に照らして」「関門を通す」と添えられている。実装からタスクへ戻る矢印があり、「収束：仕様と突き合わせ、残りをタスクに足して繰り返す」と書かれている。上には、すべての段を貫く帯として「憲章（プロジェクトの原則）が、すべての段に効く」が描かれている。下に「一段ごとに書類が残るので、ずれたときはコードではなく書類に戻って直せる」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
  ...
  <text x="246" y="310" text-anchor="middle" font-size="12.5" fill="#1E5A48" font-weight="700">聞き返すのは3件まで</text>
  <text x="246" y="330" text-anchor="middle" font-size="12.5" fill="#1E5A48" font-weight="700">残りは推測して前提を書く</text>
  ...
  <path d="M 726 280 L 726 352 L 580 352 L 580 282" fill="none" stroke="#1E5A48" stroke-width="3" marker-end="url(#sk-arrow)" />
  <text x="640" y="374" text-anchor="middle" font-size="11.5" fill="#1E5A48" font-weight="700">収束：仕様と突き合わせ、残りをタスクに足して繰り返す</text>
...
キャプション: AIに渡すのは一言の依頼ではなく、段階を踏んで固めた書類。推測そのものは止めない。**どこを推測で埋めたか**を残させるのが、この道具の立て方だ。
...
一言で頼んで直させる、を繰り返しているなら、ずれの原因は多くの場合「書いていなかったこと」にある。仕様の段で、**AIがどこを推測で埋めたかが「前提」として書き出される**ので、ずれに気づく手がかりが、作る前に残る。
...
**変化が速い。** 1.0 に到達したが、README は「いまやただの番号」とする中心開発者の投稿を引き、価値は安定から変化への適応へ移ったと説明している。手順やコマンド名が変わりうる前提で使いたい。導入には Python 3.11 以上と uv（または pipx）、Git が要る。
