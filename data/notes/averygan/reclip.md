...
自分のマシンで動かす動画・音声のダウンローダ。ブラウザで開いた画面にURLを貼ると、MP4（映像）か MP3（音声だけ）で落ちてくる。複数のURLをまとめて貼れて、重複したものは自動でまとめられる。画質を選ぶ欄もある。対応サイトが「1000以上」と広いのは、**中で yt-dlp を呼んでいる**から。形式の変換は ffmpeg が担う。
...
<svg viewBox="0 0 800 450" role="img" aria-label="見出しに「足しているのは、画面と受付だけ」とある図。左に「ブラウザの画面（URLを貼る）」があり、矢印で「ReClip（約150行の受付）localhost:8899」につながる。そこから点線の枠「すでにある道具」に伸び、中に「yt-dlp（1000以上のサイト）」と「ffmpeg（形式の変換・結合）」が並ぶ。さらに右へ「MP4 / MP3（手元のファイル）」に至る。下に「薄い部分だけを自分で持つので、対応サイトの追随は yt-dlp の更新にそのまま乗る」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
  ...
  <text x="514" y="294" text-anchor="middle" font-size="11.5" fill="#17160F" fill-opacity="0.55">（形式の変換・結合）</text>
...
**規模と体制は小さい。** 参加している人は4名ほどで、リリースのタグはまだ無く、未解決のものが issue とプルリクエスト合わせて50件あまり残っている。薄い作りなので壊れても追いやすいが、**長く面倒を見てもらえる前提では置かないほうがよい。** ライセンスは MIT。

**GitHub の言語判定は HTML になっている。** 画面が1ファイルにまとまっているためと見られ、実際に読む中身は Python とその中の JavaScript になる。
