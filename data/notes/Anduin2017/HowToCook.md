...
料理のレシピ集です。中身はコードではなく文章で、README には閲覧用の Web サービスを Docker で立てる手順も添えられています。
...
書き方が全編で揃っているのが肝です。材料を分量つきで先に全部並べ、手順は番号を振って、時間は「1分」のように数字で書く。火加減は「大火」「中火」「小火」と語で書かれていることが多い。書き方のひな形は「適量」「少量」「適当」といった曖昧な語を禁じ、**待つ工程には所要時間か、終わったと判断する基準を必ず書く**と定めている。読み手が途中で判断を求められる箇所を減らす、という方向で統一されています。そしてこれが個人のメモで終わらず、**コミュニティ運営のリポジトリ**として作られています。レシピの追加や修正は、プルリクエストで入ってきます。料理の手順をレビューして取り込む、という光景がそこにあります。

## 図

<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「レシピを同じ形に揃える」とある図。左に「よくあるレシピ（書き方がばらばら）」があり、適量・お好みで と書かれている。中央は「決まった書き方（材料が先、分量は数字）」で、300g / 5min と書かれている。右は「みんなで足す（プルリクエストで追加）」。下に「〜 形を決めたから、他人が足せるようになった 〜」とある。">
...
**スターの多さは実用性の証明ではありません。** 動かして確かめてから星を付ける類のものではないので、「それだけの人が使って動作を確認した」という意味にはなりません。着眼点の面白さに票が集まっている面が大きいと考えたほうがいいです。
