...
書き方が全編で揃っているのが肝です。材料を分量つきで先に全部並べ、手順は番号を振って、時間は「1分」のように数字で書く。火加減は「大火」「中火」「小火」と語で書かれていることが多い。書き方のひな形は「適量」「少量」「適当」といった曖昧な語を禁じ、**待つ工程には所要時間か、終わったと判断する基準を必ず書く**と定めている。読み手が途中で判断を求められる箇所を減らす、という方向で統一されています。そしてこれが個人のメモで終わらず、**コミュニティ運営のリポジトリ**として作られています。レシピの追加や修正はプルリクエストで入ってきて、フォークは1万を超えています。料理の手順をレビューして取り込む、という光景がそこにあります。
