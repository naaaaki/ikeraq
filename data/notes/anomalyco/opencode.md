...
OpenCode は、コードを書く作業を任せられるエージェントを、**特定の会社のモデルに縛られない形で**用意したもの。入口は3つあり、ターミナルの画面、デスクトップアプリ（ベータ）、エディタの拡張から使える。導入はインストール用のスクリプトのほか、npm・Homebrew・Scoop・Chocolatey・pacman・mise・Nix と、環境に合わせた経路が README に並んでいる。
...
もうひとつは**権限を分けた2つのモード**。`build` は編集も実行もできる通常のモード、`plan` は読むだけのモードで、ファイルの変更を既定で拒否し、コマンドを実行する前に許可を求める。この2つは Tab キーで切り替える。加えて `@general` で呼び出す、調べもの用のサブエージェントが内蔵されている。中身は用途ごとに分かれた多数のパッケージ（ターミナル画面・デスクトップ・サーバー・SDK・プラグインなど）で構成されていて、ドキュメントの範囲も広く、エディタの補完機能との連携、外部ツールをつなぐ MCP、実行の許可設定、会話の共有、配色、GitHub や GitLab との連携までが並ぶ。


## 図

<svg viewBox="0 0 800 450" role="img" aria-label="OpenCode の構成図。左に入口（ターミナル・デスクトップ・エディタ拡張）、中央に本体（build は編集あり、plan は読むだけ）、右に差し替えられるモデル（75以上の提供元と手元のモデル）が並び、左から右へ矢印がつながっている。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
  ...
  <text x="108" y="228" text-anchor="middle" font-size="12.5" fill="#17160F" fill-opacity="0.78">デスクトップアプリ</text>
...
**デスクトップアプリはベータと明記されている。** 常用するなら、まずターミナル版から入るほうが無難だ。ターミナルで使う場合、ドキュメントは WezTerm・Alacritty・Ghostty・Kitty といった現代的な端末を前提として挙げている。手元の端末によっては表示が崩れる可能性がある。

**名前が紛らわしい。** README は、名前に「opencode」を含む関連プロジェクトを作る場合、開発チームとは無関係である旨を明記するよう求めている。検索で出てくるものが本家とは限らない、という状態が実際にある。
