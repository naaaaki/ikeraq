...
<svg viewBox="0 0 800 450" role="img" aria-label="見出しに「書く前に、書かずに済むかを上から確かめる」とある図。左に「（例）日付を選ぶ欄がほしい」があり、中央に 1 そもそも要るか、2 もうコードの中にあるか、3 標準ライブラリにあるか、4 実行環境の機能にあるか、5 入れてある依存にあるか、6 1行で済むか が縦に並び、どれでもなければ、最小限を書く と添えてある。右は「いちばん短い答え（例）ブラウザ標準の日付入力」。下に「どの段でも削らない：」の並びと、締めの一行「手を抜いた箇所には印を残させるので、あとで直すべき場所が埋もれない」がある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
