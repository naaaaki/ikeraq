...
<svg viewBox="0 0 800 430" role="img" aria-label="見出しに「答えを1行目に、締めは次の一手」とある図。左は「いつもの返事（答えが埋もれる）」で、いい質問ですね…、状況を整理すると…、やること、ついでに言うと…、お役に立てば幸いです が並ぶ。中央は「i-have-adhd（10の決まり）」で、会話の間ずっと効く と書かれている。右は「書き換えた返事（そのまま動ける）」で、次の一手（1行目）、1. ファイルを開く、2. 関数を差し替える、3. テストを流す、2分でできる一手（締め）が並ぶ。下に「最初と最後の行だけ読めば、次に何をするかが分かる形にそろえる」とある。" style="width: 100%; height: auto; display: block; font-family: var(--jp);">
  <defs>
...
**どう発動するかは道具によって違う。** 明示的に呼び出す形のほか、エージェント側が必要と判断して自分で読みにいく形もあり、導入手順では道具ごとに書き分けられている。常に効かせる方法も、道具ごとに「任意」として用意されています（Claude Code なら目印のファイルを1つ置く、ほかの道具では決まりを設定ファイルに書き写す）。常時有効にしても「説明して」と頼めば十分に説明する、という例外は残ります。決まりを自分好みに変えたい場合は、リポジトリを複製して指示書を書き換える方式になっている。
