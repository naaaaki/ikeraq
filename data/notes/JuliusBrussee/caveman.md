...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「出ていく言葉と入ってくる資料を、別々に削る」とある図。上段は「出ていく言葉（説明・前置き）」が「スキル（言い回しだけ短く）」を通って「短い返事（10問平均 −65%）」になる。下段は「入ってくる資料（JSON・ログ・差分）」が「プロキシ（元は手元の SQLite に）」を通って「読む量が減る（6ケース・18組の比較で −33%）」になる。下に「〜 コード・コマンド・エラーの文面には手を付けない 〜」とある。">
    ...
    <text x="667" y="330" text-anchor="middle" style="font-family: var(--jp); font-size: 15px; fill: #6E6A5F;">（6ケース・18組の比較で −33%）</text>
...
削りしろが大きいのは入力側です。CSV から異常値を探すような、同じ形が延々続くデータで効きが大きいと計測されています。

## 注意点

**数字は条件つきです。ここを外して読むと期待しすぎます。** 65% は「出力だけ」で「10個のプロンプト」の平均で、内訳は 22%〜87% と開きがあります。しかもスキル自体が毎ターン 1〜1.5k の入力トークンを食うので、もともと短いやりとりが中心なら**足が出ます**。プロキシの 33.2% も、6つのケースを1ケースあたり3回ずつ、圧縮あり・なしの18組で比べた合計で（ベンチ全体は他社の道具も含む3条件・計54回）、HTML では圧縮が効かず +9.9% と増えた例が併記されています。作者自身も、削減額を約束するものではない、数字を人に伝える前に自分の環境で測ってほしい、と断っています。
