...
大規模言語モデルを**動かして配る側**のためのサーバーです。担当するのは推論のほうで、GPU1枚の構成から大規模な分散クラスタまでを想定して作られています。ただし学習の外にいるわけではありません。README は「強化学習と事後学習の土台」という項目を立てて、**多くの最前線モデルの学習で、文章を生成する担当（ロールアウト）として使われている**と書いています。AReaL、slime、verl といった事後学習の枠組みから呼ばれる形です。非営利のオープンソース団体 LMSYS のもとで公開されているもので、README には40万を超える GPU で稼働していると書かれています。開発は特定の1社の持ち物ではなく、コミュニティで進んでいます（README に並んでいる企業名は、開発への参加ではなく導入の一覧です）。

速さの中心にあるのが **RadixAttention** です。README での説明は「前置きの使い回しのための仕組み」の一行だけですが（木構造の話は開発元 LMSYS の解説に出てきます）、要点は次のところにあります。実際のリクエストは、先頭が同じことが多くあります。同じシステムプロンプトが毎回付く、同じ会話の続きが何度も送られてくる、同じ文書について質問が並ぶ——このとき、共通している部分の計算は毎回まったく同じものになります。SGLang は計算済みの状態を木の形で持っておき、**枝分かれした先だけを計算します。** 前置きが長いほど、削れる量が増えます。

残りは、詰まりやすい場所を1つずつ潰す作りです。リクエストをまとめる処理から CPU 側の待ちを取り除く、入力を読む段階と、答えを1語ずつ生成する段階を、別々の計算機に分ける、次に来る語をまとめて先読みする、長い入力を分割して流す、といったものが並びます。動く先も広く、NVIDIA の GPU のほか、AMD の GPU（MI355／MI300）、Intel Xeon の CPU、Google の TPU、Ascend の NPU が挙がっています。

## 図

<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「同じ前置きは、一度だけ計算する」とある図。左に「届いたリクエスト（先頭がそろっている）」が並び、「RadixAttention（計算済みの状態を木で持つ）」を通って、「共通の前置き」を幹に、質問A・質問B・質問C が枝として分かれる。枝の手前に「計算するのは」「この先だけ」と添えられている。下に「〜 前置きが長いほど、捨てられる計算が増える 〜」とある。">
...
**自分でサーバーを用意して運用する人向けの道具です。** モデルの重みを持ち、サーバーを立て、面倒を見る立場になって初めて効きます。外部の API を呼ぶだけの使い方もできますが、そのときはここで書いた速さの話は効きません。
