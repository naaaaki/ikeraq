...
DHH が作っている Linux ディストリビューションです。中身は Arch Linux が土台で、そこにタイル型ウィンドウマネージャの Hyprland と、デスクトップの部品を組み立てる Quickshell が載っています。その上に、Neovim、Chromium、Obsidian、LibreOffice、Kdenlive、OBS Studio といった道具が最初から入った状態で配られます。README はいま、自らを「美しく、楽しく、**エージェント志向の**ディストリビューション」と名乗っています。
...
**AI のコーディングエージェントを一級の住人として扱う**、というのが新しく前に出てきた側面です。特定の1つを推すのではなく、`claude`・`codex`・`opencode`・`copilot`・`grok` など主要なエージェントのコマンドがあらかじめ用意されていて、**最初に呼んだときに初めて本体が落ちてくる**仕組みになっています。増やしたいものがあれば同じ形で包めます。
...
<figure>
  <svg viewBox="0 0 800 450" role="img"
       aria-label="見出しに「同じ4つを積む。誰が選ぶかだけが違う」とある図。左は「ふつうに組む場合」で、アプリ … どれを入れる？／見た目 … 何で作る？／窓の管理 … 何を使う？／土台 … どのディストロ？ が並び、（4回ぶんの判断が利用者に残る）と添えてある。右は「Omarchy」で、Neovim・Obsidian ほか一式／Quickshell／Hyprland／Arch Linux が積まれ、（4つとも作者が決めてある・ISO1枚）と書かれている。下に「〜 減っているのは選択肢ではなく、選ぶ手間のほう 〜」とある。">
...
設定を詰めはじめると、本来やりたかった作業が後回しになります。ウィンドウマネージャの選定から配色の調整まで済んだ状態が、遅い機械でも5分ほどで立ち上がるなら、その時間はまるごと省けます。あとから気に入らない部分を直せばよい、という順番にできます。
...
**キーボードの種類に制約があります。** 既定の構成ではディスク暗号化が有効なので、起動時にパスワードを入力する必要があります。マニュアルは「Bluetooth キーボードからは暗号化のパスワードを入力できない」と明記しています。有線か 2.4GHz の無線が要ります。
