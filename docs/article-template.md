...
先頭の設定欄に書く。`npm run note` の冒頭に本数と未コミットの記事が出る。

| 値 | 意味 | いつ変える |
|---|---|---|
| `draft` | 書いている途中 | 下書きを作った時点で入っている |
| `review` | Naoki の確認待ち | 事実確認を通して Naoki に見せたとき |
| `skip` | 見送り | 公開しないと決めたとき。理由を先頭のコメントに1行残し、**コミットしない**（コミットするとサイトに載る） |

**公開するときは `status` の行を消してからコミットする。** 無い記事は公開済みとみなす。
サイトの表示は `status` を見ていないので、書き忘れても表示は変わらない。
...
- `aria-label` は**図に書いた文字を写す**。見出しを冒頭、締めの一行を末尾に必ず入れる
  （書き下ろすとずれる。公開済み20本を確認したら20本ともずれていた。手順は `CLAUDE.md` 4-7）
